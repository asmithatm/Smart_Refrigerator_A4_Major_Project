...
public class expiryServlet extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        out.println("<title>Get Expiry Date</title>");
        out.println("<link rel='stylesheet' href='expiry.css'>");
        out.println("</head>");
        out.println("<body>");
        ...
        String encmsg="v2.0#S#30#001#02#I#"+category+"&"+subCategory+"&"+brand+"&"+item+"*";        
        try {
			String serverMsg = TCPClient.client(encmsg);
	        out.println("<div class='items'>"+serverMsg+"</div>");
	        request.setAttribute("answer", serverMsg);
	        RequestDispatcher disp = request.getRequestDispatcher("expiry.html");
